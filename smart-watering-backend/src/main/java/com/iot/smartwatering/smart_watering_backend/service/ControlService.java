...
public class ControlService {
    ...
    public String controlWatering(ControlRequest request) {
        ...
        String pump = request.getPump().toUpperCase();

        if ("ON".equals(pump)) {
            return startWatering(zone, request.getDurationMinutes(), false, null, request.getTargetHumidity());
        } else if ("OFF".equals(pump)) {
            return stopWatering(zone);
        } else {
            throw new RuntimeException("Invalid pump command: " + pump);
        }
    }
    ...
    public String startWateringWithWeatherCheck(Long zoneId, Integer durationMinutes, String location) {
        ...
        return startWatering(zone, durationMinutes, true, location, null);
    }
    ...
    private String startWatering(Zone zone, Integer durationMinutes, boolean checkWeather, String location,
        ...
        // Send MQTT command
        mqttService.publishControlCommand(zone.getZoneId(), "on", targetHumidity);
        ...
        logUserAction(zone, checkWeather ? UserAction.ActionType.AUTO_WATER_ON : UserAction.ActionType.MANUAL_WATER_ON,
                actionDetail);
    ...
    private String stopWatering(Zone zone) {
        ...
        // Send MQTT command
        mqttService.publishControlCommand(zone.getZoneId(), "off", 0);
